# SPEC.md: Custom VDOM JS Library

Спецификация — **контракт поведения** библиотеки. По этому документу можно реализовать библиотеку с нуля, не глядя на текущий код: другой алгоритм diff, другая структура instance, другие имена функций — но **поведение для пользователя должно совпадать**.

Библиотека: компактная альтернатива React на чистом JavaScript (ES6+). Использует собственный VDOM, diff/patch алгоритм, кастомный жизненный цикл, динамическое дерево контекстов, порталы с отложенным монтированием.

Название `tyaff` используется только в документации. В коде использовать общие названия: `VDOM`, `Component`, `h`, `mount`.
...
- Массивы в `children` **остаются массивами** (не flattening-уются)
- `null`, `undefined`, `false` → не создают DOM-узел, но **сохраняют позицию** (не «схлопывают» место). Это нужно, чтобы идентификаторы последующих элементов не менялись при условном рендере.
- **Текстовые узлы обязательно оборачиваются в объекты** (`{ _text: String(value) }`)

**Почему массивы не flattening-уются:** каждый массив — это отдельный уровень в дереве идентификаторов. В React `[['a', 'b'], ['c']]` flattening-уется в `['a', 'b', 'c']`, и UID элемента `'c'` = `,2`. Здесь массивы сохраняют структуру — UID для `'c'` в `[['a', 'b', ['c']]]` = `,2,0` (индекс во внешнем массиве + индекс во внутреннем). Это даёт стабильные идентификаторы при изменении внутренней структуры массива.
...
Пользователь не должен переопределять в `definition` имена, которые движок использует для API компонента:
- lifecycle: `init`, `render`, `props`, `memo`, `onMounted`, `onUpdated`, `onUnmounted`
- context: `context`, `contextSelf`
- API instance: `update`, `refs`

Также не использовать имена, начинающиеся с `_` — движок хранит под ними внутреннее состояние.

### Архитектурные ограничения

- **НЕТ двусторонних ссылок**: компонент хранит своё последнее отрендеренное дерево (вниз), но **не** обратную ссылку на свой vnode. Портал хранит своё отрендеренное содержимое, не свой vnode.
- **Блокировка rerender в `init()`**: если `init()` вызывает `update()`, rerender блокируется. Первый render — строго после `init()`.
- **`init()` выполняется только при первом создании instance**. При повторном рендере компонента в том же месте с тем же типом `init()` **не выполняется** — только обновляются props и запускается внутренний rerender.
...
4. Первый rerender → `memo(this.props)` → `render(this.props)`
...
**Методы:**
- `this.context(key, ...args)` — **всегда** обращается к родительскому контексту
...
**Распространение:** при рендере движок прокидывает ссылку на ближайший контекст-провайдер вниз по дереву. HTML-теги прозрачно передают ссылку (не являются провайдерами, но и не разрывают цепочку).
...
//       └─ div           прозрачная передача контекста
...
- **Защиту от повторного входа** в рендер того же компонента (если уже внутри — игнорировать)
- **Защиту от вызова `update()` внутри `render()`** — в development mode выводить `console.error`. **Почему нельзя:** `render()` ещё не закончился, дерево vnode в неопределённом состоянии, попытка обновления может привести к зацикливанию. Рекомендация: использовать прямое присваивание (`this.value = 22`).
- **Защиту от бесконечных циклов обновлений** (например, `onUpdated` триггерит новый `update()`) — лимит на количество обновлений в одной задаче, при превышении `console.error` и очистка очереди.

### Batching

Несколько `update()` в одном тике объединяются в один render. Конкретный механизм (microtask, макротаска и т.д.) — на усмотрение реализации.

### Изоляция ошибок

В development mode каждый компонент в очереди обновлений обрабатывается в своём `try/catch`:
- Если компонент падает в `render()` — ошибка логируется с именем компонента, компонент не выполняется
- Остальные компоненты в очереди продолжают обновляться

В production mode изоляция отключается для максимальной производительности (ошибка в одном компоненте может прервать batch).
...
### Первый рендер идёт через механизм обновления
- Первый render **обязан** идти через тот же путь, что и последующие обновления (а не напрямую через `render()`)
- Это обеспечивает: `memo()` работает с первого рендера, `onUpdated` не выполняется при первом mount
...
- При каждом ререндере заново выполняется `containerGetter()`:
  - Контейнер появился → монтаж (первый раз)
  - Контейнер тот же → точечный дифф детей
  - Контейнер сменился → узлы **перемещаются** из старого контейнера в новый (без размонтирования, сохраняются instance и state)
...
- Одинаковые компоненты (тот же тип компонента — та же фабрика) → сохранение instance, обновление контекста и пропсов, запуск `props() → memo()`
...
User keys должны быть уникальны в пределах одного render. Дубликаты — ошибка разработчика, но движок обрабатывает их детерминированно:
...
- В development mode выводится `console.warn` о duplicate key
- В production mode предупреждение не выводится, но поведение то же

Это гарантирует что duplicate keys не ломают рендер: каждый элемент получает свой идентификатор, переиспользование работает корректно.

**Рекомендация:** не полагаться на это поведение. Duplicate keys — ошибка, которую нужно исправить в коде приложения.
...
**null-плейсхолдеры сохраняют позицию:**
`null`/`undefined`/`false` в children не создают DOM-узел, но **сохраняют свою позицию** в массиве. Это значит, что при условном рендере идентификаторы последующих элементов не меняются.

```javascript
render() {
    return h('div', null,
        this.show && h('span', null, 'content'),  // null если !show, но позиция занята
        h('p', null, 'always here')               // UID = ',1' независимо от this.show
    );
}
```

Рекомендуется использовать условный рендер через `&&` именно так — null-плейсхолдер в нужной позиции. **Не** рекомендуется `render()` возвращать `null` на верхнем уровне как способ «спрятать» компонент, а потом вернуть vnode — это другой кейс (полное размонтирование/монтаж).
...
**Обновление критических атрибутов (React-like поведение):**
При изменении критических атрибутов (`type`, `multiple`, `is`) элемент **не пересоздаётся**. Движок просто обновляет атрибут через DOM API (`setAttribute`/`removeAttribute`):
- `type` у `<input>` — обновляется динамически (ограничения браузера: нельзя менять на/с `type="file"`)
- `multiple` у `<select>` — обновляется как обычный boolean-атрибут
- `is` у custom elements — обновляется через `setAttribute`
...
**Поведение:**
- Находит все смонтированные деревья и их корневые компоненты
...
- Предупреждение о дубликатах ключей (`console.warn`)
- Изоляция ошибок компонентов (try/catch)
- Подробные сообщения об ошибках

**Production mode:**
- Отключение предупреждения о дубликатах ключей (но логика обработки та же — первый key выигрывает)
