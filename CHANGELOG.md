...
---
### 2026-07-06 — Критическое обновление алгоритма reconciliation

**Полная переработка diff-алгоритма для соответствия спецификации.**

### Контекст

В ходе разработки было выявлено критическое расхождение между реализацией и SPEC.md. Старый алгоритм reconciliation не соответствовал контрактам, описанным в спецификации, что требовало полной замены подхода. Работа велась несколько дней и затронула ядро библиотеки.

### Что изменилось

**Новый алгоритм reconciliation** (подробное описание в `docs/doc-reconciliation.md`):

- **Однопроходный алгоритм** — вместо классического двухпроходного подхода (отдельный reconcile + синхронизация DOM) всё выполняется за один проход
- **Персистентный keyMap** — карта старых элементов хранится на instance/container и обновляется инкрементально, не перестраивается каждый раз
- **Идентификаторы элементов** — два типа:
  - `#key` для элементов с `props.key` (могут перемещаться между позициями)
  - Путь (например, `,0,1,2`) для элементов без key (привязаны к позиции)
- **Наследование идентификаторов** — дети keyed-элементов наследуют `#key` как префикс, что позволяет переиспользовать их при перемещении родителя
- **Инлайн вызовы** — `ref()` и `onMounted()` вызываются прямо в момент создания/переиспользования узла, без отдельных обходов дерева
- **Cleanup через Set** — после основного прохода удаляются неиспользованные элементы из keyMap с вызовом lifecycle hooks

### Влияние на производительность

Переход на новый алгоритм привёл к снижению производительности библиотеки. Это ожидаемо для экспериментального проекта, где процесс разработки и соответствие спецификации важнее оптимизации.

**Текущий статус:**
- Частично удалось вернуть показатели к прежним значениям
- Продолжается работа над оптимизациями
- Поведение полностью соответствует SPEC.md
- Все тесты проходят

### Технические детали

- Удалён старый двухпроходный алгоритм
- Переписаны функции обработки HTML-элементов, компонентов, Fragment и Portal
- Изменена логика выстраивания детей в DOM
- Обновлена система ключей для поддержки наследования идентификаторов
- Добавлена обработка duplicate keys с предупреждениями в development mode

### Документация

- Создан `docs/doc-reconciliation.md` с детальным описанием нового алгоритма
- Добавлены примеры и сравнение с классическим подходом
- Описаны ключевые отличия и компромиссы производительности
